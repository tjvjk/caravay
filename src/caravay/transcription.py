...
from caravay.artifacts import HASHES, remove, useful
from caravay.models import inspect
from caravay.repetition import trim
from caravay.settings import LANGUAGES, Backend, Settings
from caravay.translation import Format, Outcome, ValidationError
...
def validate(root: Path, verbose: bool) -> Path:
    ...
    if state == "missing":
        raise ValidationError(
            "model_not_installed: run caravay models download before transcription"
        )
    if state == "invalid":
        raise ValidationError(
