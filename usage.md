...
uv run caraway run --source kaz --target rus meeting.wav | tee translation.txt
```
...
```console
uv run caraway run --source hye --target eng --format jsonl armenian.m4a > result.jsonl
```
...
```console
printf 'Բարեւ' | uv run caraway translate --source hye --target eng -
```
...
`~/Library/Application Support/caraway/config.toml`. Models are stored in
`~/Library/Caches/caraway` by default. To change the cache location:
...
Run `uv run caraway models download` to install, resume an interrupted download,
...
| `model_not_installed` | Run `uv run caraway models download` |
...
```console
uv run caraway live --help
uv run caraway run --source kaz --target rus --verbose meeting.wav > translation.txt
```
