# Caravay offline Armenian translation MVP

Label: wayfinder:map

## Destination

An implementation-ready MVP specification for `caravay`, validated by an acceptance prototype on the user's Apple-Silicon Mac: Eastern Armenian audio files and text become English text locally after model download through the `transcribe`, `translate`, and `run` commands.
...
- Production implementation of the `caravay` CLI; this effort ends with its validated specification.
