...
**What to build:** Make `caravay models download` acquire, verify, and atomically
publish the one pinned SeamlessM4T Large v2 snapshot into Caravay's managed cache,
...
**State:** closed
**Closed by:** https://github.com/tjvjk/caravay/pull/2

- [x] `caravay [--config PATH] models download` accepts no model name and is the
      only command permitted to fetch model data.
- [x] The command downloads the processor plus speech-to-text and text-to-text
      files for the exact packaged repository revision into temporary state on the
      destination filesystem.
- [x] Publication requires a manifest with expected byte sizes and cryptographic
      digests, verifies every digest, and atomically makes the snapshot ready.
- [x] Interrupted transfers retain every completed file that the standard Hugging
...
- [x] A per-snapshot lock rejects a concurrent downloader with
      `download_in_progress`, status 1, and no primary stdout.
- [x] Starting or resuming a transfer, or preparing invalid-snapshot replacement,
      requires at least 20 GiB free; the ready idempotent path does not.
- [x] An invalid snapshot is replaced through recoverable quarantine and atomic
      publication; failed publication restores it, and a later invocation recovers
      quarantine left by process interruption.
- [x] A network, storage, or verification failure reports `download_failed`,
      returns 1, and neither publishes incomplete data nor damages a ready snapshot.
- [x] An already ready snapshot returns 0 without a network request; all success
      paths keep stdout empty and send progress only to interactive stderr.
- [x] Successful publication retains no second complete model copy in the managed
      cache and remains observable as `ready` through ticket 10's command.
- [x] Subprocess tests exercise success, resume, locking, invalid replacement,
      recovery, insufficient space, failure safety, and idempotence through local
      controlled download fixtures.

## Decisions

- Caravay remains a thin Unix-style orchestrator around
  ...
  staged files. Caravay does not implement its own HTTP or Range downloader.
- Caravay owns only its managed-cache contract: the immediate per-snapshot lock,
