...
to capture the complete system-audio mix and continuously writes Caravay's existing
live PCM format to standard output. Piping that output to `caravay live` must turn
...
**State:** closed
**Closed by:** https://github.com/tjvjk/caravay/pull/8
...
PCM format, exits directly from a signal handler, and does not provide Caravay's
...
- [x] Successful capture writes only headerless little-endian Float32 mono PCM at
      exactly 16 kHz to stdout, matching `caravay live --input-format f32le -`
...
- [x] An opt-in acceptance test on the reference Mac plays a known Source Armenian
      ...
      directly into `caravay live --input-format f32le -`, and obtains useful
...
Merged in [PR #8](https://github.com/tjvjk/caravay/pull/8) on 2026-09-07.
