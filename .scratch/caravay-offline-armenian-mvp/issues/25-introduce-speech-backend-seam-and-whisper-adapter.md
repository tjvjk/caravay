...
- [ ] Caravay's canonical ISO 639-3 codes are mapped to and from Whisper's language
...
- [ ] The direct multilingual live route from issue 24 can select Whisper with an
      ...
      `caravay live --route fused --backend whisper-large-v3 --target eng --input-format f32le -`
...
- [ ] Text output remains only useful final text. JSONL records the selected
      ...
      new schema version.
- [ ] `caravay models status` reports every packaged backend independently, and
      `caravay models download BACKEND` downloads only the selected immutable
