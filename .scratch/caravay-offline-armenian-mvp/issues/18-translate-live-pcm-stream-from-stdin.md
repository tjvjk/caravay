...
- [x] The CLI exposes an explicit command such as
      `caravay live --input-format f32le -`; it does not change the finite-file
      behavior or input contract of `caravay run` and `caravay transcribe`.
...
- [x] The documented local-file simulation uses an equivalent of
      `ffmpeg -re -i INPUT -f f32le -ac 1 -ar 16000 pipe:1 | caravay live --input-format f32le -`,
      so audio reaches Caravay at approximately capture speed rather than being
