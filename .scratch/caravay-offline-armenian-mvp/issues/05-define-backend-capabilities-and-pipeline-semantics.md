...
How should `caravay` represent ASR, text translation, and fused speech translation capabilities so `transcribe`, `translate`, and `run` compose predictably without assuming either one model per stage or an intermediate transcript?
