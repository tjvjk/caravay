...
**What to build:** Establish a runnable development version of `caravay` and make
`caravay models status` report whether the pinned managed model snapshot is ready,
...
**State:** closed
**Closed by:** https://github.com/tjvjk/caravay/pull/1

- [x] The development environment uses Python `>=3.13,<3.14`, pins PyTorch 2.14.0
      and Transformers 5.16.1, and exposes a runnable `caravay` command without
      claiming distribution packaging.
- [x] `caravay [--config PATH] models status` accepts no model name and writes
