...
**State:** closed
**Closed by:** https://github.com/tjvjk/caravay/pull/9
...
```console
native/SystemAudioCapture/.build/release/caravay-capture \
    | uv run caravay live --input-format f32le -
```
...
- [x] On a compatible interactive terminal, `caravay live` presents one transient
...
- [x] Plain transcript stdout remains byte-compatible with the existing immutable
      LF-delimited contract. JSONL output and PCM stdout from `caravay-capture`
...
- [x] Startup and terminal rendering do not assume that the native capture
      producer is present: `caravay live` provides useful status when fed by any
      valid paced PCM producer, and `caravay-capture` remains independently usable.
