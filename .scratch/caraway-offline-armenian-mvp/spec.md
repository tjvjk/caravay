# Caraway offline Source Armenian translation MVP
...
and throwaway model prototypes, but no durable `caraway` command that can be
...
Build a macOS command-line application named `caraway` with three processing
commands:
...
10. As a shell user, I want translation to read stdin until EOF, so that I can
    pipe Source Armenian text into Caraway.
...
44. As a user, I want unsupported hardware or unavailable MPS reported explicitly,
    so that Caraway does not run unexpectedly slowly on CPU.
...
- An execution plan is an explicit ordered cascade of compatible capabilities,
  ...
  loading. Caraway never infers a route or substitutes a backend at runtime.
...
- The CLI forms are `caraway transcribe [OPTIONS] AUDIO_FILE`, `caraway translate
  [OPTIONS] [TEXT_FILE|-]`, and `caraway run [OPTIONS] AUDIO_FILE`.
...
- A composed `run` segment includes `source_transcript` when speech-to-text produced
  ...
  transcripts, and stages with no transcript. Caraway never fabricates it.
...
- The optional strict TOML configuration is at
  `~/Library/Application Support/caraway/config.toml`; `--config PATH` selects a
...
- The packaged model is `facebook/seamless-m4t-v2-large` at immutable revision
  `5f8cc790b19fc3f67a61c105133b20b34e3dcb76`. A Caraway release, never an automatic
  refresh, changes this revision.
- The default managed cache root is `~/Library/Caches/caraway`. A shared Hugging
  Face cache entry is not by itself an installed Caraway snapshot.
...
- `caraway models download` is the only operation allowed to fetch model data. It
...
- `caraway models status` performs local startup verification and emits exactly
...
- Stable setup reason codes are `invalid_config`, `model_not_installed`,
  ...
  directs the user to `caraway models download`.
...
- The single public test seam is invocation of `caraway` as a separate process.
