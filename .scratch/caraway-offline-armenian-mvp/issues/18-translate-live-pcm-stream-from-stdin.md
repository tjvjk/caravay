...
- [ ] The CLI exposes an explicit command such as
...
- [ ] The initial live input contract requires raw little-endian Float32 mono PCM
      ...
      loading.
- [ ] The documented local-file simulation uses an equivalent of
...
- [ ] Input is consumed incrementally in short frames and drained independently
...
- [ ] Segment timestamps are derived from the monotonic PCM sample position, not
...
- [ ] Hybrid segmentation starts a candidate on speech, closes it after a
...
- [ ] Initial defaults are documented and covered by tests: 16 kHz input, a
...
- [ ] End of stdin is orderly stream completion: any remaining speech is finalized
...
- [ ] Each completed or degraded useful Source Armenian segment is passed through
      ...
      exposed.
- [ ] Default text output contains only ordered English utterances, writes each
      ...
      stdout.
- [ ] JSONL output identifies live mode and includes, per result, the source audio
      ...
      failure.
- [ ] The bounded-buffer overload policy is explicit and deterministic. It never
...
- [ ] SIGINT and a broken input pipe stop capture, settle or cancel in-flight work
      according to one documented policy, release resources, and preserve the
      stdout/stderr and exit-status contracts.
- [ ] Fast tests use a controlled PCM producer, clock, VAD boundary, and speech and
...
- [ ] An opt-in acceptance test paces a short conversational Source Armenian
...
- [ ] Microphone and macOS system-audio capture remain separate producers outside
