...
`~/Library/Application Support/caraway/config.toml`. Caraway does not search the
...
Changing that revision is a packaged Caraway update, never an automatic cache
refresh. The default managed cache root is `~/Library/Caches/caraway`; model
snapshots live below it by backend name and revision. Caraway does not treat an
arbitrary entry in the shared Hugging Face cache as an installed model.

A ready snapshot has a Caraway manifest containing the backend name, repository,
revision, and expected files with their byte sizes and cryptographic digests. After
download, Caraway verifies every expected file and its digest before publishing the
...
```text
caraway [--config PATH] models download
caraway [--config PATH] models status
```
...
missing-model diagnostic tells the user to run `caraway models download`.
