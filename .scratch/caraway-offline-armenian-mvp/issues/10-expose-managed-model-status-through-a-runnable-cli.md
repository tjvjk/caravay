...
Implemented and merged through [PR #1](https://github.com/tjvjk/caraway/pull/1).
The development CLI now loads strict TOML settings through `pydantic-settings`,
resolves the pinned managed snapshot, and reports `ready`, `missing`, or `invalid`
through the subprocess boundary. The implementation is covered by 24 subprocess
tests and gated by Ruff, strict mypy, pre-commit, and CI. Merge commit: `f20f3a4`.
